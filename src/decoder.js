...
export function decode(buffer, port, unk) {
	let _buffer = new Buffer(buffer.length - 1);
	buffer.copy(_buffer, 0, 1);

	let checksumEncr = buffer[0];
	let bPort = (port + 1) ^ 0xCC;

	for (let i = 0; i < _buffer.length; i++) {
		if (unk === 0)
			_buffer[i] = unk ^ _buffer[i], unk++;
		else
			_buffer[i] = bPort ^ _buffer[i], unk--;

		_buffer[i] = sampDecrTable[_buffer[i]];
	}
	/*
	let checksum = 0;
	for (let i = 0; i < _buffer.length; i++)
		checksum ^= (_buffer[i] & 0xCC);
	*/
	return _buffer;
}